	...
	$.fn.redCountdown = function(Options, Callback) {
		...
		function prepareCountdown() {
			...
			if ($("#redCountdownCSS").length <= 0) {
				$("body").append("<style id='redCountdownCSS'>.redCountdownWrapper > div { display:inline-block; position:relative; width:calc(100% - 20px); margin:10px; } .redCountdownWrapper .redCountdownValue {  width:100%; line-height:1em; position:absolute; top:50%; text-align:center; left:0; display:block;}</style>");
			}
			...
			Element.find(".redCountdownSeconds input").knob($.extend({width: '100%', displayInput: false,  readOnly: true, max: 60,min:60-OnlySecondsLeft+1}, Settings.style.secondsElement.gauge));
		...
		function Responsive() {
			...
			if (Settings.style.textResponsive) {
				Element.find(".redCountdownValue").css("font-size", Math.floor(Element.find("> div").eq(0).width() * Settings.style.textResponsive / 3) + "px");
				Element.find(".redCountdownValue").each( function() {
					$(this).css("margin-top",Math.floor(0 - (parseInt($(this).height()) / 2)) + "px");
				});
			}
